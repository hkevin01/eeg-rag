...
class TestSynthesisAgent:
    ...
    @pytest.mark.asyncio
    async def test_synthesize_uses_llm_summary_when_client_is_available(self):
        """Test that the LLM path returns model output instead of the template."""
        from eeg_rag.agents.synthesis_agent import SynthesisAgent
            ...
            choices=[
                SimpleNamespace(
                    message=SimpleNamespace(
                        content="LLM summary for EEG synthesis."
                    )
                )
            ]
        )
        create_mock = AsyncMock(return_value=llm_response)
        ...
        agent = SynthesisAgent(
            ...
            config={"model": "test-model", "temperature": 0.2, "max_tokens": 256},
        )
        ...
        assert result.summary == "LLM summary for EEG synthesis."
