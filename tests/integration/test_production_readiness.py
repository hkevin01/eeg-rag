...
async def setup_system():
    """Setup complete system for testing across module classes."""
    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)

        memory_manager = MemoryManager(db_path=temp_path / "test_memory.db")
...
class TestPerformanceBenchmarks:
    ...
    async def test_query_response_time_benchmark(self, setup_system):
        ...
        benchmark_queries = [
            ("What are EEG frequency bands?", QueryComplexity.SIMPLE),
            ("How is EEG used in epilepsy diagnosis?", QueryComplexity.MODERATE),
            ("Compare different EEG artifact removal methods", QueryComplexity.COMPLEX)
        ]
        ...
        for query_text, complexity in benchmark_queries:
            query = AgentQuery(
                text=query_text,
                context={},
                parameters={"complexity": complexity.value},
            )
