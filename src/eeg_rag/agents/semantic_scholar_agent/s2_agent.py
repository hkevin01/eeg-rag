...
# =============================================================================
# ID:             MOD-S2-001
# Requirement:    REQ-S2-010 — Full Semantic Scholar API integration;
#                 REQ-S2-011 — Citation graph traversal;
#                 REQ-S2-012 — Author expertise tracking;
#                 REQ-S2-013 — Paper recommendations and influence scoring.
# Purpose:        Retrieve EEG-relevant papers from the Semantic Scholar
#                 Graph API with citation graph traversal, influence scoring,
#                 and open-access PDF discovery.
# Rationale:      Semantic Scholar provides citation-count data, influence
#                 metrics, and full field-of-study classification not available
#                 in PubMed alone. High-citation EEG papers signal consensus
#                 findings relevant to clinical decision support.
# Inputs:         AgentQuery.text (natural language);
#                 AgentQuery.parameters (max_results, year_range,
#                 fields_of_study, open_access_only).
# Outputs:        AgentResult.data = {"papers": [S2Paper.to_dict()...],
#                 "total_count": int, "returned_count": int}.
#                 Papers sorted descending by InfluenceScorer.score_paper().
# Preconditions:  S2 API accessible; optional API key for higher rate limits.
# Postconditions: Papers cached for 12 hours; influence scores attached.
# Assumptions:    S2 free tier: 100 req/5min (20/min); partner tier: higher.
# Side Effects:   HTTP GET to api.semanticscholar.org; cache growth.
# Failure Modes:  HTTP 429 → rate-limit back-off; timeout → empty result;
#                 missing abstract (common in S2) → abstract=None (not error).
# Error Handling: All exceptions caught in execute(); return AgentResult(success=False).
# Constraints:    Rate: 20 req/min (free), 100 req/min (API key);
#                 Max 100 papers per request (S2 API limit); timeout: 30s.
# Verification:   tests/test_search_validation.py; tests/test_source_health.py.
# References:     Semantic Scholar API v1 documentation; InfluenceScorer;
#                 REQ-S2-010–013.
# =============================================================================
