...
class StatsService:
    ...
    def __init__(
        ...
        if corpus_dir:
            ...
            project_root = Path(__file__).parent.parent.parent.parent  # src/eeg_rag/services -> project root
                ...
                with open(jsonl_file, 'r') as f:
                    for line in f:
                        ...
                        try:
                            ...
                            year = paper.get("year")
                            if year:
                                if result["year_min"] is None or year < result["year_min"]:
                                    result["year_min"] = year
                                if result["year_max"] is None or year > result["year_max"]:
    ...
    def get_total_papers(self, use_cache: bool = True) -> int:
        ...
        if total == 0:
            ...
            if total > 0:
                logger.info(f"Found {total} papers in corpus files: {corpus_stats['files_found']}")
        ...
        if self.papers_db_path.exists():
            ...
            with self._get_connection(self.papers_db_path) as conn:
                if conn:
                    ...
                    if 'papers' in tables:
                        # NULL paper_ids
                        cursor.execute("SELECT COUNT(*) FROM papers WHERE paper_id IS NULL")
                        null_ids = cursor.fetchone()[0]
                        if null_ids > 0:
                            report["inconsistencies"].append(f"{null_ids} papers with NULL paper_id")
                        ...
                        if duplicates:
                            report["inconsistencies"].append(f"{len(duplicates)} duplicate paper_ids")
                            report["duplicate_ids"] = [row[0] for row in duplicates[:10]]
                        
                        # Check source field
                        cursor.execute("SELECT COUNT(*) FROM papers WHERE source IS NULL OR source = ''")
                        no_source = cursor.fetchone()[0]
                        if no_source > 0:
                            report["inconsistencies"].append(f"{no_source} papers without source")
        else:
            report["inconsistencies"].append(f"Papers database not found at {self.papers_db_path}")
            report["recommendations"].append("Initialize the papers database or check the path configuration")
