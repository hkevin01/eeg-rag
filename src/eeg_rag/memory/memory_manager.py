...
class MemoryManager:
    ...
    def __init__(
        ...
        try:
            ...
            # Lightweight conversation store for async integration compatibility.
    ...
    async def store_conversation_turn(
        ...
        """Store a conversation turn (async compatibility API)."""
        conv_id = validate_non_empty_string(conversation_id, "conversation_id")
        item = {
        ...
        self._conversation_store.setdefault(conv_id, []).append(item)

        # Keep unified memory populated for retrieval/context features.
        self.add_query(query, {"conversation_id": conv_id, **(metadata or {})})
        self.add_response(response, metadata={"conversation_id": conv_id})
    ...
    async def get_conversation_history(
        ...
        """Get recent turns for a conversation (async compatibility API)."""
        conv_id = validate_non_empty_string(conversation_id, "conversation_id")
        limit = int(validate_positive_number(limit, "limit", min_value=1))
        history = self._conversation_store.get(conv_id, [])
        return history[-limit:]

    async def cleanup_old_conversations(self, max_age_days: int = 30) -> int:
        """Cleanup old conversation turns (async compatibility API)."""
        cutoff = datetime.now() - timedelta(days=max_age_days)
        removed = 0

        for conv_id, turns in list(self._conversation_store.items()):
            kept = [t for t in turns if t.get("timestamp", datetime.now()) >= cutoff]
            removed += len(turns) - len(kept)
            if kept:
                self._conversation_store[conv_id] = kept
            else:
    ...
    async def close(self) -> None:
        """Close memory manager resources (async compatibility API)."""
        return None
