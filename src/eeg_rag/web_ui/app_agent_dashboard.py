...
async def _execute_simulated(
    ...
    tracker.update_agent(
        "orchestrator", AgentStatusEnum.RUNNING, 20, "Analyzing query complexity..."
    )
    tracker.add_timeline_event("Orchestrator", f"Received query: '{query[:50]}...'")
    await asyncio.sleep(0.3)

    tracker.update_agent(
        "orchestrator", AgentStatusEnum.RUNNING, 50, "Routing to agents..."
    )
    tracker.add_timeline_event(
        "Orchestrator",
        "Query classified as RESEARCH type, routing to 5 agents",
        "success",
    )
    ...
    tracker.update_agent(
        "query_planner", AgentStatusEnum.RUNNING, 30, "Decomposing query..."
    )
    tracker.add_timeline_event(
        "Query Planner", "Breaking down query into sub-questions"
    )
    await asyncio.sleep(0.3)

    tracker.update_agent(
        "query_planner", AgentStatusEnum.RUNNING, 70, "Identifying EEG terms..."
    )
    await asyncio.sleep(0.2)
    ...
    terms_display = (
        ", ".join(eeg_terms[:5]) if eeg_terms else "EEG, classification, deep learning"
    )

    tracker.update_agent(
        "query_planner", AgentStatusEnum.SUCCESS, 100, f"Found: {terms_display}"
    )
    tracker.add_timeline_event(
        "Query Planner", f"Identified terms: {terms_display}", "success"
    )
    await asyncio.sleep(0.1)

    # Phase 3: Parallel Agent Execution
    tracker.update_agent(
        "local_agent", AgentStatusEnum.RUNNING, 10, "Searching FAISS index..."
    )
    tracker.update_agent(
        "pubmed_agent", AgentStatusEnum.RUNNING, 5, "Querying PubMed API..."
    )
    tracker.update_agent(
        "semantic_scholar", AgentStatusEnum.RUNNING, 5, "Querying S2 API..."
    )

    tracker.add_timeline_event(
        "Local Agent", "Searching 125,000 indexed paper embeddings"
    )
    ...
    tracker.update_agent(
        "local_agent", AgentStatusEnum.RUNNING, 50, "Found 45 candidates..."
    )
    await asyncio.sleep(0.3)
    tracker.update_agent(
        "local_agent",
        ...
        100,
        f"Retrieved {max_sources + 5} papers",
        max_sources + 5,
    )
    tracker.add_timeline_event(
        "Local Agent",
        f"Found {max_sources + 5} papers with avg similarity 0.82",
        "success",
    )
    ...
    await asyncio.sleep(0.2)
    tracker.update_agent(
        "pubmed_agent", AgentStatusEnum.RUNNING, 60, "Parsing XML responses..."
    )
    await asyncio.sleep(0.3)
    tracker.update_agent(
        "pubmed_agent", AgentStatusEnum.SUCCESS, 100, "Retrieved 12 papers", 12
    )
    tracker.add_timeline_event(
        "PubMed Agent", "Found 12 papers from last 5 years with PMIDs", "success"
    )

    # Semantic Scholar progress
    tracker.update_agent(
        "semantic_scholar", AgentStatusEnum.RUNNING, 40, "Expanding citations..."
    )
    await asyncio.sleep(0.2)
    tracker.update_agent(
        "semantic_scholar", AgentStatusEnum.SUCCESS, 100, "Retrieved 8 papers", 8
    )
    tracker.add_timeline_event(
        "Semantic Scholar", "Found 8 highly-cited papers (>100 citations)", "success"
    )

    # Knowledge graph (may or may not be available)
    tracker.update_agent(
        "graph_agent", AgentStatusEnum.RUNNING, 20, "Querying Neo4j..."
    )
    await asyncio.sleep(0.3)
    tracker.update_agent(
        "graph_agent", AgentStatusEnum.SUCCESS, 100, "Found 5 relationships", 5
    )
    tracker.add_timeline_event(
        "Knowledge Graph",
        "Found concept relationships: EEG→seizure→prediction",
        "success",
    )
    ...
    tracker.update_agent(
        "orchestrator", AgentStatusEnum.RUNNING, 70, "Validating citations..."
    )
    tracker.update_agent(
        "citation_agent", AgentStatusEnum.RUNNING, 30, "Verifying PMIDs..."
    )
    tracker.add_timeline_event(
        "Citation Agent", "Validating 30 citations against PubMed"
    )
    await asyncio.sleep(0.4)

    tracker.update_agent(
        "citation_agent", AgentStatusEnum.RUNNING, 70, "Checking retractions..."
    )
    await asyncio.sleep(0.3)
    tracker.update_agent(
        "citation_agent", AgentStatusEnum.SUCCESS, 100, "25 verified, 5 excluded", 25
    )
    tracker.add_timeline_event(
        "Citation Agent", "Validated 25 papers (2 retracted, 3 no PMID)", "success"
    )
    tracker.update_metrics(citations_verified=25)

    # Phase 5: Aggregation
    tracker.update_agent(
        "aggregator", AgentStatusEnum.RUNNING, 40, "Merging results..."
    )
    tracker.add_timeline_event("Aggregator", "Deduplicating and ranking 25 papers")
    await asyncio.sleep(0.3)

    tracker.update_agent(
        "aggregator", AgentStatusEnum.RUNNING, 80, "Ranking by relevance..."
    )
    await asyncio.sleep(0.2)
    tracker.update_agent(
        "aggregator", AgentStatusEnum.SUCCESS, 100, f"Top {max_sources} selected"
    )
    tracker.add_timeline_event(
        "Aggregator",
        f"Selected top {max_sources} papers (removed 5 duplicates)",
        "success",
    )

    # Phase 6: Response Generation
    tracker.update_agent(
        "synthesis_agent", AgentStatusEnum.RUNNING, 20, "Building context..."
    )
    tracker.add_timeline_event(
        "Response Synthesizer", "Generating response with Mistral/GPT-4"
    )
    await asyncio.sleep(0.3)

    tracker.update_agent(
        "synthesis_agent", AgentStatusEnum.RUNNING, 60, "Synthesizing answer..."
    )
    await asyncio.sleep(0.4)

    tracker.update_agent(
        "synthesis_agent", AgentStatusEnum.RUNNING, 90, "Adding citations..."
    )
    await asyncio.sleep(0.2)
    ...
    tracker.update_agent(
        ...
        AgentStatusEnum.SUCCESS,
        100,
        f"Generated with {max_sources} citations",
    )
    tracker.add_timeline_event(
        "Response Synthesizer",
        f"Generated comprehensive answer with {max_sources} inline citations",
        "success",
    )
    ...
    tracker.add_timeline_event(
        "Orchestrator", f"Query completed in {total_time:.0f}ms", "success"
    )
...
def main():
    ...
    <div class="main-header">
        ...
        <div style="display: flex; gap: 16px; margin-top: 12px;">
            ...
            <span style="color: #C4B5FD; font-size: 0.85rem;">125K cached | 200M+ searchable (PubMed • S2 • arXiv • OpenAlex • CrossRef)</span>
            <span style="color: #7C3AED;">|</span>
    ...
    with col_main:
        ...
        if query and not st.session_state.current_result:
            ...
            st.markdown(
                render_query_understanding(query, query_info), unsafe_allow_html=True
            )
        ...
        if search_btn and query:
            ...
            with st.spinner(
                "🤖 Agents working... Watch the activity panel on the right →"
            ):
        ...
        if st.session_state.current_result:
            ...
            if result.get("success"):
                ...
                st.success(
                    f"✅ Query completed in {latency/1000:.2f}s • {len(result.get('sources', []))} sources retrieved • {metrics.get('citations_verified', 0)} citations verified"
                )
                ...
                with tab_response:
                    ...
                    if current_query:
                        ...
                        st.markdown(
                            render_query_understanding(current_query, query_info),
                            unsafe_allow_html=True,
                        )
                ...
                with tab_analysis:
                    ...
                    confidence_data = {
                        "overall": 0.85,
                        "retrieval": min(
                            0.95, 0.7 + len(result.get("sources", [])) * 0.05
                        ),
                        "citation": (
                            0.95 if metrics.get("citations_verified", 0) > 0 else 0.5
                        ),
                        "synthesis": 0.80,
                    }
                    st.markdown(
                        render_confidence_breakdown(confidence_data),
                        unsafe_allow_html=True,
                    )
                    ...
                    llm_config = {
                        "model": (
                            "Mistral-7B-Instruct"
                            if not result.get("used_openai")
                            else "GPT-4o-mini"
                        ),
                        "temperature": 0.3,
                        "max_tokens": 1500,
                        "context_tokens": min(
                            4096, len(result.get("response", "")) * 2
                        ),
                    }
                    st.markdown(
                        render_llm_info_panel(llm_config), unsafe_allow_html=True
                    )
                    ...
                    if timeline_data:
                        st.markdown(
                            render_timeline(timeline_data), unsafe_allow_html=True
                        )
                    else:
                ...
                with tab_sources:
                    ...
                    if sources:
                        ...
                        total_citations = sum(
                            s.get("citation_count", 0) for s in sources
                        )
                    ...
                    for i, source in enumerate(sources, 1):
                        with st.expander(
                            f"📄 [{i}] {source.get('title', 'Unknown')[:70]}... ({source.get('year', 'N/A')})",
                            expanded=(i <= 2),
                        ):
                            ...
                            with col1:
                                ...
                                st.markdown(
                                    f"**Authors:** {source.get('authors', 'Unknown')}"
                                )
                                ...
                                if source.get("architecture"):
                                    st.markdown(
                                        f"**Architecture:** {source['architecture']}"
                                    )
                                if source.get("accuracy"):
                                    st.markdown(
                                        f"**Reported Accuracy:** {source['accuracy']}"
                                    )
                            ...
                            with link_col3:
                                title_query = source.get("title", "").replace(" ", "+")[
                                    :50
                                ]
                                st.link_button(
                                    "🎓 Scholar",
                ...
                with tab_technical:
                    ...
                    with metrics_cols[1]:
                        st.metric(
                            "Papers Searched", f"{metrics.get('papers_searched', 0):,}"
                        )
                    with metrics_cols[2]:
                        st.metric(
                            "Papers Retrieved", f"{metrics.get('papers_retrieved', 0)}"
                        )
                    with metrics_cols[3]:
                        st.metric(
                            "Citations Verified",
                            f"{metrics.get('citations_verified', 0)}",
                        )
                    with metrics_cols[4]:
