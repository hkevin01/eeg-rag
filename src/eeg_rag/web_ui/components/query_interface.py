...
def determine_query_type(query: str) -> str:
    ...
    if any(word in query_lower for word in ["compare", "difference", "versus", "vs"]):
        ...
        word in query_lower
        for word in ["clinical", "diagnosis", "treatment", "patient"]
    ):
        return "clinical"
    else:
        return "factual"
...
def render_comprehensive_response(
    ...
    # Get papers and filter by relevance - STORE IN SESSION STATE
    # Check if we need to regenerate or use cached results
    cache_key = f"{query}_{max_sources}_{relevance_threshold}"
    if (
        "last_query_cache_key" not in st.session_state
        or st.session_state.last_query_cache_key != cache_key
    ):
        all_papers = generate_mock_papers(query, max_sources)
        filtered_papers = [
            p for p in all_papers if p["relevance_score"] >= relevance_threshold
        ]
        # Store in session state
        st.session_state.last_query_cache_key = cache_key
        st.session_state.current_filtered_papers = filtered_papers
        st.session_state.current_query = query
        st.session_state.current_query_id = query_id
    else:
        # Reuse cached results for pagination
        filtered_papers = st.session_state.get("current_filtered_papers", [])
...
def render_papers_summary(papers: list, threshold: float):
    ...
    if not papers:
        st.warning(
            f"No papers found above {threshold:.0%} relevance threshold. Try lowering the threshold."
        )
        return
...
def render_paper_card(paper: dict, rank: int):
    ...
    with st.expander(
        f"**#{rank}** {paper['title'][:70]}... | PMID:{paper['pmid']} | {paper['relevance_score']:.0%} Relevance",
        expanded=(rank <= 3),
    ):
        ...
        with col3:
            st.markdown(
                f"📈 Relevance: **{paper['relevance_score']:.0%}** ({rel_text})"
            )
        with col4:
        ...
        st.markdown(
            f"**{paper['journal']}** ({paper['year']}) {paper['volume']}{':' + paper['pages'] if paper['pages'] else ''}"
        )
        ...
        st.markdown(
            f"**📊 Study Details:** {paper['study_type']} | Sample: {paper['sample_size']}"
        )
...
def render_pagination(current_page: int, total_pages: int):
    ...
    num_cols = min(total_pages + 2, 12)  # prev + pages + next
    ...
    for page in pages_to_show:
        ...
        with cols[col_idx]:
            if page == "...":
                st.markdown(
                    "<div style='text-align: center; padding: 0.5rem;'>...</div>",
                    unsafe_allow_html=True,
                )
            else:
                if page == current_page:
                    st.markdown(
                        f"<div style='text-align: center; padding: 0.5rem; background: #1976D2; color: white; border-radius: 4px; font-weight: bold;'>{page}</div>",
                        unsafe_allow_html=True,
                    )
                else:
                    if st.button(
                        str(page), key=f"page_{page}", use_container_width=True
                    ):
